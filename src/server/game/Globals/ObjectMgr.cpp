...
bool ObjectMgr::LoadFrenchCOREStrings(char const* table, int32 min_value,
		int32 max_value) {
	...
	for (FrenchCoreStringLocaleMap::iterator itr = mFrenchCoreStringLocaleMap.begin();
			itr != mFrenchCoreStringLocaleMap.end();) {
		if (itr->first >= start_value && itr->first < end_value)
			mFrenchCoreStringLocaleMap.erase(itr++);
		else
			++itr;
	}
	...
	if (!result) {
		sLog->outString();
		if (min_value == MIN_frenchcore_string_ID) // error only in case internal strings
			sLog->outErrorDb(
					">> Loaded 0 FRENCHCORE Strings. DB table `%s` is empty. Cannot continue.",
					table);
		else
			sLog->outString(
	...
	do {
		...
		FrenchCoreStringLocale& data = mFrenchCoreStringLocaleMap[entry];
	...
	sLog->outString();
	if (min_value == MIN_frenchcore_string_ID)
		sLog->outString(">> Loaded %u FRENCHCORE Strings from table %s", count,
				table);
	else
...
const char *ObjectMgr::GetFrenchCOREString(int32 entry,
		LocaleConstant locale_idx) const {
	if (FrenchCoreStringLocale const *msl = GetFrenchCOREStringLocale(entry)) {
	...
	if (entry > 0)
		sLog->outErrorDb("Entry %i not found in `frenchcore_string` table.",
				entry);
	else
...
void ObjectMgr::AddSpellToTrainer(uint32 entry, uint32 spell, uint32 spellCost,
		...
		uint32 KillCredit) {
	if (entry >= FRENCHCORE_TRAINER_START_REF)
		return;
...
void ObjectMgr::CheckScripts(ScriptsType type, std::set<int32>& ids) {
	...
	for (ScriptMapMap::const_iterator itrMM = scripts->begin();
		...
		for (ScriptMap::const_iterator itrM = itrMM->second.begin();
			...
			case SCRIPT_COMMAND_TALK: {
				if (!GetFrenchCOREStringLocale(itrM->second.Talk.TextID))
					sLog->outErrorDb(
...
void ObjectMgr::LoadDbScriptStrings() {
	LoadFrenchCOREStrings("db_script_string", MIN_DB_SCRIPT_STRING_ID,
			MAX_DB_SCRIPT_STRING_ID);

	std::set<int32> ids;

	for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
		if (GetFrenchCOREStringLocale(i))
			ids.insert(i);
...
bool LoadFrenchCOREStrings(char const* table, int32 start_value, int32 end_value) {
	...
	return sObjectMgr->LoadFrenchCOREStrings(table, start_value, end_value);
}
