...
namespace Trinity
{
    class BattlegroundChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetFrenchCOREString(_textId, loc_idx);
                if (_args)
                {
    ...
    class Battleground2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetFrenchCOREString(_textId, loc_idx);
                char const* arg1str = _arg1 ? sObjectMgr->GetFrenchCOREString(_arg1, loc_idx) : "";
                char const* arg2str = _arg2 ? sObjectMgr->GetFrenchCOREString(_arg2, loc_idx) : "";
...
{
    const char *format = sObjectMgr->GetFrenchCOREStringForDBCLocale(entry);
...
// To be removed
const char* Battleground::GetFrenchCOREString(int32 entry)
{
    // FIXME: now we have different DBC locales and need localized message for each target client
    return sObjectMgr->GetFrenchCOREStringForDBCLocale(entry);
}
