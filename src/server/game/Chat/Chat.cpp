...
const char *ChatHandler::GetFrenchCOREString(int32 entry) const
{
    return m_session->GetFrenchCOREString(entry);
}
...
{
    SendSysMessage(GetFrenchCOREString(entry));
}

void ChatHandler::PSendSysMessage(int32 entry, ...)
{
    const char *format = GetFrenchCOREString(entry);
    va_list ap;
    char str [2048];
    va_start(ap, entry);
...
const char *CliHandler::GetFrenchCOREString(int32 entry) const
{
...
{
    return GetFrenchCOREString(LANG_CONSOLE_COMMAND);
}
