...
bool ChatHandler::HandleModifyRepCommand(const char * args)
{
    ...
    if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
    {
        ...
        {
            std::string rank = GetFrenchCOREString(ReputationRankStrIndex[r]);
            if (rank.empty())
                continue;
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    std::string username = GetFrenchCOREString(LANG_ERROR);
    std::string email = GetFrenchCOREString(LANG_ERROR);
    std::string last_ip = GetFrenchCOREString(LANG_ERROR);
    uint32 security = 0;
    std::string last_login = GetFrenchCOREString(LANG_ERROR);
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetFrenchCOREString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleCharacterReputationCommand(const char* args)
{
    ...
    for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
    {
        ...
        std::string rankName = GetFrenchCOREString(ReputationRankStrIndex[rank]);
...
bool ChatHandler::HandleLookupEventCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < events.size(); ++id)
    {
        ...
        if (Utf8FitTo(descr, wnamepart))
        {
            ...
            char const* active = activeEvents.find(id) != activeEvents.end() ? GetFrenchCOREString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleEventActiveListCommand(const char* /*args*/)
{
    ...
    char const* active = GetFrenchCOREString(LANG_ACTIVE);
...
bool ChatHandler::HandleEventInfoCommand(const char* args)
{
    ...
    char const* activeStr = active ? GetFrenchCOREString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleWintergraspStatusCommand(const char* /*args*/)
{
   ...
   if (!pvpWG || !sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
    ...
    PSendSysMessage(LANG_BG_WG_STATUS, sObjectMgr->GetFrenchCOREStringForDBCLocale(
...
bool ChatHandler::HandleWintergraspSwitchTeamCommand(const char* /*args*/)
{
   OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197);
    ...
    PSendSysMessage(LANG_BG_WG_SWITCH_FACTION, GetFrenchCOREString(pvpWG->getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
    return true;
}
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetFrenchCOREString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? GetFrenchCOREString(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    char const* knownStr = GetFrenchCOREString(LANG_KNOWN);
