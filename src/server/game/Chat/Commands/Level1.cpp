...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetFrenchCOREString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetFrenchCOREString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetFrenchCOREString(LANG_GM_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetFrenchCOREString(LANG_VISIBLE) : GetFrenchCOREString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetFrenchCOREString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(const char* args)
{
    ...
    sLog->outDetail(GetFrenchCOREString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(const char* args)
    {
        ...
        if (addmoney < 0)
        {
            ...
            sLog->outDetail(GetFrenchCOREString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
            if (newmoney <= 0)
            {
        ...
        sLog->outDetail(GetFrenchCOREString(LANG_NEW_MONEY), moneyuser, addmoney, target->GetMoney());
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetFrenchCOREString(LANG_ON) : GetFrenchCOREString(LANG_OFF));
        return true;
    }
...
/*    MapEntry const * me = sMapStore.LookupEntry(tele->mapId);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetFrenchCOREString(LANG_OFFLINE), tele->name.c_str());
