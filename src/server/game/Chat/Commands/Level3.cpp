...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetFrenchCOREString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetFrenchCOREString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    sLog->outDetail(GetFrenchCOREString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog->outDetail(GetFrenchCOREString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                {
                    knownStr = GetFrenchCOREString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetFrenchCOREString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetFrenchCOREString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetFrenchCOREString(LANG_TALENT);
                if (passive)
                    ss << GetFrenchCOREString(LANG_PASSIVE);
                if (learn)
                    ss << GetFrenchCOREString(LANG_LEARN);
                if (known)
                    ss << GetFrenchCOREString(LANG_KNOWN);
                if (active)
                    ss << GetFrenchCOREString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetFrenchCOREString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetFrenchCOREString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetFrenchCOREString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetFrenchCOREString(index);

                    ss << ' ' << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ')';

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetFrenchCOREString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetFrenchCOREString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetFrenchCOREString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetFrenchCOREString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetFrenchCOREString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetFrenchCOREString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetFrenchCOREString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetFrenchCOREString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetFrenchCOREString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetFrenchCOREString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetFrenchCOREString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetFrenchCOREString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetFrenchCOREString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetFrenchCOREString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetFrenchCOREString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetFrenchCOREString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetFrenchCOREString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetFrenchCOREString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetFrenchCOREString(LANG_TALENT);
    char const* passiveStr = GetFrenchCOREString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetFrenchCOREString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetFrenchCOREString(LANG_BANINFO_YES) : GetFrenchCOREString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetFrenchCOREString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetFrenchCOREString(LANG_BANINFO_YES) : GetFrenchCOREString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetFrenchCOREString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetFrenchCOREString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
