...
void WorldSession::SendNotification(uint32 string_id, ...) {
	char const* format = GetFrenchCOREString(string_id);
	if (format) {
		va_list ap;
		char szStr[1024];
		szStr[0] = '\0';
...
const char * WorldSession::GetFrenchCOREString(int32 entry) const {
	return sObjectMgr->GetFrenchCOREString(entry, GetSessionDbLocaleIndex());
}
