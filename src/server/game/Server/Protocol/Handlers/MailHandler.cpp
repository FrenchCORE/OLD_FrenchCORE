...
void WorldSession::HandleSendMail(WorldPacket & recv_data) {
	...
	if (pl->getLevel() < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ)) {
		SendNotification(GetFrenchCOREString(LANG_MAIL_SENDER_REQ),
	...
	if (receiveLevel < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ)) {
		SendNotification(GetFrenchCOREString(LANG_MAIL_RECEIVER_REQ),
				sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
		return;
	}
			...
			if (GetSecurity() > SEC_PLAYER
				...
				if (receive) {
					...
					if (!sObjectMgr->GetPlayerNameByGUID(sender_guid,
							sender_name))
						sender_name = sObjectMgr->GetFrenchCOREStringForDBCLocale(
								LANG_UNKNOWN);
				}
				sLog->outCommand(
						GetAccountId(),
