...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
	...
	if (type == CHAT_MSG_AFK)
	{
		...
		if ((msg.empty() || !_player->isAFK()) && !_player->isInCombat())
		{
			if (!_player->isAFK())
			{
				if (msg.empty()) msg = GetFrenchCOREString(
...
// also check SPELL_AURA_COMPREHEND_LANGUAGE (client offers option to speak in that language)
	...
	else
	{
		...
		if (!_player->CanSpeak())
		{
			...
			SendNotification(GetFrenchCOREString(LANG_WAIT_BEFORE_SPEAKING),
					timeStr.c_str());
			return;
		}
	...
	if (GetPlayer()->HasAura(1852) && type != CHAT_MSG_WHISPER)
	{
		...
		SendNotification(GetFrenchCOREString(LANG_GM_SILENCE),
				GetPlayer()->GetName());
		return;
	}
	...
	switch (type)
	{
		...
		case CHAT_MSG_YELL:
		{
			if (GetSecurity() == SEC_PLAYER)
			{
				...
				{
					SendNotification(GetFrenchCOREString(LANG_SAY_REQ),
							sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
					return;
				}
			}
		...
		case CHAT_MSG_WHISPER:
		{
			...
			{
				SendNotification(GetFrenchCOREString(LANG_WHISPER_REQ),
						sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
				return;
			}
		...
		case CHAT_MSG_CHANNEL:
		{
			...
			// {
			// SendNotification(GetFrenchCOREString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
			// return;
			// }
			// }
