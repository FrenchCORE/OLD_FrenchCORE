    ...
    else if (obj->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        if (eventId == obj->GetGOInfo()->building.damagedEvent)
        {
            ...
            switch(state->type)
            {
                ...
                {
                    msgStr = fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_WORKSHOP_DAMAGED), msgStr.c_str(), sObjectMgr->GetFrenchCOREStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                }
                    break;
                case BUILDING_WALL:
                {
                    sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_FORTRESS_UNDER_ATTACK));
                ...
                case BUILDING_TOWER:
                {
                    ...
                    msgStr = fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_TOWER_DAMAGED), msgStr.c_str());
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
                        if (getDefenderTeam()==TEAM_ALLIANCE)
        ...
        else if (eventId == obj->GetGOInfo()->building.destroyedEvent)
        {
            ...
            switch(state->type)
            {
                case BUILDING_WORKSHOP:
                {
                    ...
                    msgStr = fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_WORKSHOP_DESTROYED), msgStr.c_str(), sObjectMgr->GetFrenchCOREStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                }
                    break;
                case BUILDING_WALL:
                ...
                case BUILDING_TOWER:
                {
                    ...
                    msgStr = fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_TOWER_DESTROYED), msgStr.c_str());
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
                        if (getDefenderTeam()==TEAM_ALLIANCE)
    ...
    if (m_timer > diff)
    {
        ...
        if (isWarTime())
        {
            if (m_timer != 1) // 1 = forceStopBattle
                sWorld->SendZoneText(ZONE_WINTERGRASP, fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(entry), sObjectMgr->GetFrenchCOREStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
            EndBattle();
        } else {
            if (m_timer != 1)
                sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_BATTLE_STARTS));
            StartBattle();
        }
...
{// Uptime will do all the work
    ...
    {
        m_timer = 1;
        sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_START));
    }
    sWorld->SendWintergraspState();
}
...
{// Uptime will do all the work.
    ...
    {
        m_timer = 1;
        sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_STOP));
    }
       sWorld->SendWintergraspState();
}
...
void OutdoorPvPWG::forceChangeTeam()
{
    ...
    sWorld->SendZoneText(ZONE_WINTERGRASP, fmtstring(sObjectMgr->GetFrenchCOREStringForDBCLocale(LANG_BG_WG_SWITCH_FACTION), sObjectMgr->GetFrenchCOREStringForDBCLocale(getAttackerTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
    if (isWarTime())
        forceStartBattle();
    else
        forceStopBattle();
